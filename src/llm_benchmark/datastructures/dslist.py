...
class DsList:
    ...
    This class provides various list manipulation operations including
    modification, searching, sorting, reversing, rotating, and merging.
...
# For backward compatibility, provide module-level functions that delegate to the class
def modify_list(v: List[int]) -> List[int]:
    """Modify a list by adding 1 to each element (backward compatibility wrapper)"""
    return DsList.modify_list(v)


def search_list(v: List[int], n: int) -> List[int]:
    """Search a list for a value (backward compatibility wrapper)"""
    return DsList.search_list(v, n)


def sort_list(v: List[int]) -> List[int]:
    """Sort a list of integers (backward compatibility wrapper)"""
    return DsList.sort_list(v)


def reverse_list(v: List[int]) -> List[int]:
    """Reverse a list of integers (backward compatibility wrapper)"""
    return DsList.reverse_list(v)


def rotate_list(v: List[int], n: int) -> List[int]:
    """Rotate a list of integers (backward compatibility wrapper)"""
    return DsList.rotate_list(v, n)


def merge_lists(v1: List[int], v2: List[int]) -> List[int]:
    """Merge two lists of integers (backward compatibility wrapper)"""
